...
export const Timeline = component$(() => {
  ...
  const _currentYearRef = useSignal<HTMLElement>()
  ...
  useVisibleTask$(async ({ cleanup }) => {
    ...
    if (!state.isMobile) {
      ...
      const scrollTween: GSAPTween = gsap.to(trackRef.value, {
  ...
  return (
    <section ref={containerRef} class='timeline-section'>
      ...
      <div class='timeline-container'>
        <div ref={trackRef} class='timeline-track'>
          ...
          {[2022, 2023, 2024, 2025, 2026].map((year) => {
            ...
            return (
              <div key={year} class='year-marker' style={`left: calc(${xPosition}px + 50vw);`}>
          ...
          {Array.from({ length: 48 }, (_, i) => {
            ...
            return (
              !isYearMarker && (
                <div key={i} class='month-marker' style={`left: calc(${xPosition}px + 50vw);`}>
      ...
      {/* Mobile scroll hint */}
      {state.isMobile && <div class='mobile-hint'>{t("app.timeline.mobile_hint")}</div>}

      {/* Event Carousel */}
      <EventCarousel events={timelineData} currentYear={state.currentYear} currentMonth={state.currentMonth} />
    </section>
  )
})
