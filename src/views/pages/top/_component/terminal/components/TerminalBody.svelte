<script lang="ts">
  ...
  import VimEditor from './VimEditor.svelte'
...
<div class="terminal-body" bind:this={terminalBody}>
  ...
  {:else}
    {#each $terminalStore.displayedLines as line (line.id)}
      <div 
        class="terminal-line {line.type === 'log' || line.type === 'info' || line.type === 'system' || line.type === 'success' ? 'log-line' : ''}"
    ...
    {#if $terminalStore.isTyping}
      <div class="terminal-line" style="color: {getLineColor('command')}">
        <span class="prompt">$</span> {$currentCommand}<span class="cursor">_</span>
      </div>
    {/if}
  {/if}
</div>
