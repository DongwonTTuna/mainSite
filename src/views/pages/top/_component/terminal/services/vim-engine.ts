...
export class VimEngine {
  ...
  private registerDefaultCommands() {
    ...
    this.registerCommand({
      ...
      action: (state) => ({
        ...
        cursorCol: Math.min(state.cursorCol, state.content[Math.min(state.content.length - 1, state.cursorRow + 1)]?.length || 0)
      })
    })
  ...
  processInsertModeKey(key: string, state: VimState): Partial<VimState> {
    ...
    if (key === 'Backspace') {
      if (state.cursorCol > 0) {
        newContent[state.cursorRow] = 
          currentLine.substring(0, state.cursorCol - 1) + 
          currentLine.substring(state.cursorCol)
        return {
          content: newContent,
    ...
    // Regular character
    newContent[state.cursorRow] = 
      currentLine.substring(0, state.cursorCol) + 
      key + 
      currentLine.substring(state.cursorCol)
  ...
  processCommandModeKey(key: string, state: VimState): Partial<VimState> {
    ...
    if (key === 'Enter') {
      ...
      return this.executeVimCommand(command, state)
    }
  ...
  private executeVimCommand(command: string, state: VimState): Partial<VimState> {
